...
const MentionSuggestions = ({ textAreaRef }) => {
  ...
  if (!showSuggestions || mentionSuggestions.length === 0) return null;
  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        exit={{ opacity: 0, y: -10 }}
        className="fixed z-50 bg-white rounded-lg shadow-lg border border-gray-200 w-64 max-h-60 overflow-y-auto"
        style={{
        ...
        <ul className="py-1">
          {mentionSuggestions.map((user, index) => (
            <li
              ...
              onClick={() => insertMention(user, textAreaRef)}
            >
