...
const {FiMap,FiPlus,FiCalendar,FiEdit3,FiTrash2,FiCheckCircle,FiAlertCircle,FiX,FiLoader,FiTag,FiClock,FiTrendingUp,FiTarget,FiSearch,FiFilter,FiMessageCircle,FiSend,FiChevronDown,FiChevronUp,FiThumbsUp,FiPaperclip}=FiIcons;
...
const Roadmap=()=> {
  ...
  const handleSubmit=async (e)=> {
    ...
    try {
      const tagsArray=form.tags ? form.tags.split(',').map(tag=> tag.trim()).filter(tag=> tag.length > 0) : [];
      const roadmapData={
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-start justify-between mb-4 gap-4">
                    <div className="flex-1">
                      <div className="flex flex-wrap items-center space-x-3 mb-2">
                        <h3 className="text-lg sm:text-xl font-semibold text-gray-900 hover:text-blue-600 transition-colors cursor-pointer"
                      ...
                      {userProfile ? (
                        <div className="space-y-3">
                          <div className="relative">
                            ...
                            />
                            <MentionSuggestions textAreaRef={{current: commentTextAreaRefs.current[item.id]}} />
                          </div>
