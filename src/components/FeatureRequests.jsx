...
const FeatureRequests = () => {
  ...
  const handleSubmitFeature = async (e) => {
    ...
    if (!userProfile) {
      setStatusMessage({
        type: 'error',
        message: 'You must be logged in to submit a feature request'
      });
      return;
    }
    ...
    try {
      ...
      setStatusMessage({
        type: 'error',
        message: 'Failed to submit feature request: ' + error.message
      });
    } finally {
    ...
    try {
      ...
      if (hasVoted) {
        ...
        setVotes(prev => ({
          counts: {
            ...prev.counts,
            [featureId]: Math.max(0, (prev.counts[featureId] || 0) - 1)
          },
      ...
      <div className="p-6">
        ...
        <div className="space-y-4">
          ...
          {filteredFeatures.length === 0 && (
            ...
            >
              <SafeIcon icon={FiStar} className="text-gray-400 text-6xl mx-auto mb-4" />
